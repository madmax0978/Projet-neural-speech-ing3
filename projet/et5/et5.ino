  ...
  for (int i = 0; i < NUM_FRAMES; i++) {
    ...
    for (int j = 0; j < FRAME_SIZE; j++) {
      if (start + j < RECORD_SIZE) {
        ...
        frames[i][j] = 0; // padding pour éviter les pertes 
}
    }
  }
}
  ...
  if (recording && sampleReady) {
    ...
    if (++downsampleCounter == 4) {  // 32kHz → 8kHz
      ...
      if (recordedSamples >= RECORD_SIZE) {
        ...
        extractFrames();
        printFrames();
        Serial.println("== FIN FRAMES ==");

        // Ajout pour vérification recouvrement
        Serial.println("== VERIFICATION RECOUVREMENT ==");
        for (int i = 0; i < 20; i++) {
          Serial.print("Frame0[");
          Serial.print(128 + i);
          Serial.print("] = ");
          Serial.print(frames[0][128 + i]);
          Serial.print(" | Frame1[");
          Serial.print(i);
          Serial.print("] = ");
          Serial.println(frames[1][i]);
        }
